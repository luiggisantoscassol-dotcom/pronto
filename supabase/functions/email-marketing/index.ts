...
Deno.serve(async (request) => {
  ...
  if (authError || !auth.user) return json({ error: "Faça login novamente no painel." }, 401);
  ...
  if (action === "overview") {
    ...
    return json({ ok: true, inscritos: count || 0, campanhas: campaigns || [] });
  }
  const campaign = {
  ...
  if (campaign.nome.length < 3 || campaign.assunto.length < 3 || campaign.conteudo.length < 10) return json({ error: "Preencha nome, assunto e conteúdo da campanha." }, 400);
  const from = Deno.env.get("RESEND_FROM") || "Tio Nan <pedidos@tionan.com.br>";
  if (action === "test") {
    const to = String(input.email_teste || auth.user.email || "").trim();
    if (!validEmail(to)) return json({ error: "Informe um e-mail válido para o teste." }, 400);
  ...
  if (!subscribers.length) return json({ error: "Nenhum cliente autorizou e-mail marketing." }, 409);
  ...
  try {
    ...
    return json({ ok: true, destinatarios: subscribers.length, broadcast_id: broadcast.id });
  } catch (error) {
